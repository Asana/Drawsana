  ...
  ShapeSelectable
{
  ...
  public func render(in context: CGContext) {
    ...
    if isStraight {
      ...
      let isAngleWithGrondLarger45Degree = CGFloat.pi/4 <= angle && angle <= 3 * CGFloat.pi/4
      let c = isAngleWithGrondLarger45Degree ?  CGPoint(x: a.x, y: b.y) : CGPoint(x: b.x, y: a.y)
